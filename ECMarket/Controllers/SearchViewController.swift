...
//  ECMarket
//
//  Created by MacBook Pro on 2020/08/08.
...
class SearchViewController: UIViewController {
    ...
    private func searchInFirebase(forName: String) {
        ...
        searchAlogolia(searchString: forName) { (itemIds) in
    ...
    @objc func textFieldDidChange(_ textField: UITextField) {
        ...
        if searchButtonOutlet.isEnabled {
            searchButtonOutlet.backgroundColor = #colorLiteral(red: 0.03531464934, green: 0.7925925255, blue: 1, alpha: 1)
        } else {
            disableSearchButton()
        }
    }
